...
def attempt(subset):
    ...
    for n,names,bits in groups:
        ...
        candidates=np.flatnonzero(np.all(((first>=32)&(first<127))|(first==0),axis=1))
        for index in candidates:
            ...
            raise SystemExit(0)
    if len(seen)%10000==0:print('Tested',len(seen),'sets in',round(time.monotonic()-started,1),'s',flush=True)
